...
from yarp_adapter import build_execute_envelope, retry_envelope, validate_yarp_envelope
...
class YarpAdapterTests(unittest.TestCase):
    ...
    def test_retry_changes_transport_identity_not_semantic_identity(self):
        first = build_execute_envelope(self.execution, sender_id="agent-chatgpt-ag", receiver_id="agent-test-executor")
        second = retry_envelope(first)
        self.assertEqual("EXECUTE_MP", first.message_type)
